...
const UpdateArtistCalendar = ({idCalendar}) => {

    const classes = useStyles();
    const router = useRouter();
    const {id} = router.query;
    //const {data: calendar, mutate, error} = useSWR(`/calendars/${idCalendar}`, fetcher);
    const {data: calendar, error, mutate} = useSWR(`/calendars/${idCalendar}`, fetcher);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            mutateIndex(`/artists/${id}/calendars`);
            //mutate();
...
export default UpdateArtistCalendar;