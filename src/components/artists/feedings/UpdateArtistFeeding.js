...
const UpdateArtistFeeding = ({id, idArtist}) => {

    const classes = useStyles();
    const router = useRouter();
    //const {id} = router.query;
    const {data: feeding, error, mutate} = useSWR(`/artists/${idArtist}/feedings/${id}`, fetcher);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            mutateIndex(`/artists/${idArtist}/feedings`);
            mutate();
            handleClose();
            setUpdateInfo(true);
        } catch (error) {
...
export default UpdateArtistFeeding;