...
const DeleteLodgingArtist = ({idArtist, idLodging}) => {

    const classes = useStyles();
    const router = useRouter();
    //const {id} = router.query;
    //const {data: lodgingArtist, error} = useSWR(`/lodgings/${idLodging}/artists/${idArtist}`, fetcher);
    ...
    const handleDelete = async () => {
        try {
            setProcessing(true);
            await LodgingArtist.delete(idLodging, idArtist);
    ...
    return (
        <div>
            ...
            >
                <DialogTitle id="alert-dialog-title">{"¿Deseas remover este artista del hospedaje?"}</DialogTitle>
                <DialogActions>
