...
const DeleteUserCalendar = ({idCalendar, idUser}) => {
    ...
    const handleDelete = async () => {
        try {
            setProcessing(true);
            await UserCalendar.delete(idUser, idCalendar);
            setDeleteSuccess(true);
            handleClose();
            mutateTo(`/users/${idUser}/calendars`);
        } catch (error) {
            setDeleteError(true);
            setProcessing(false);
            handleClose();
    ...
    return (
        <div>
            ...
            >
                <DialogTitle id="alert-dialog-title">{"¿Deseas remover este calendario del usuario?"}</DialogTitle>
                <DialogActions>
...
export default DeleteUserCalendar;
