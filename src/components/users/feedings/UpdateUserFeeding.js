...
const UpdateUserFeeding = ({id, idUser}) => {

    const classes = useStyles();
    const {data: feeding, error, mutate} = useSWR(`/users/${idUser}/feedings/${id}`, fetcher);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            mutateIndex(`/users/${idUser}/feedings`);
            mutate();
            handleClose();
            setUpdateInfo(true);
        } catch (error) {
...
export default UpdateUserFeeding;