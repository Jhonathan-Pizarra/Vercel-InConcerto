...
const UpdateFestivalConcert = ({id, idFestival}) => {

    const router = useRouter();
    // const {id} = router.query;
    const classes = useStyles();
    //const {data: festivalConcert, error, mutate} = useSWR(`/concerts/${id}`, fetcher); ///festivals/${id}/concerts/${id}
    const {data: festivalConcert, error, mutate} = useSWR(`/festivals/${idFestival}/concerts/${id}`, fetcher);
    ...
    if(!festivalConcert) return <Loading/>
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            await Concert.update(id, {
                ...data,
                name: ((data.name) === "") ? `Vacío (${festivalConcert.id})` : data.name,
            ...
            mutateIndex(`/festivals/${idFestival}/concerts`);
            handleClose();
            setUpdateInfo(true);
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <DialogContent>
                        ...
                        <TextField
                            ...
                            type="text"
                            margin="dense"
                            defaultValue={festivalConcert.name}
                            {...register('name')}
                            fullWidth
                        />
                    ...
                    <DialogContent>
                        ...
                        <Select
                            ...
                            defaultValue={festivalConcert.festival_pk}
                            onChange={handleChangeFestival}
                            {...register("festival_id")}
                        >
                            <option key={festivalConcert.festival_pk}  value={festivalConcert.festival_pk} disabled={true}>{festivalConcert.festival}</option>
                    ...
                    <DialogContent>
                        ...
                        <Select
                            ...
                            defaultValue={festivalConcert.place_pk}
                            onChange={handleChangePlace}
                            {...register("place_id")}
                        >
                            <option key={festivalConcert.place_pk}  value={festivalConcert.place_pk} disabled={true}>{festivalConcert.place}</option>
...
export default UpdateFestivalConcert;